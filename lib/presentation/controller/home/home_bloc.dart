...
class HomeBloc extends Bloc<BaseHomeEvent, HomeState> {
  ...
  HomeBloc(this.getCategoriesDataUseCase, this.getProductsDataByCategoryUseCase,
    ...
    on<GetCategoriesEvent>((event, emit) async {
      ...
      result.fold(
          ...
          (r) => emit(state.copyWith(
              categoriesData: r, categoriesDataState: RequestState.loaded)));
    });
    on<GetProductsDataByCategoryEvent>((event, emit) async {
      final result =
          await getProductsDataByCategoryUseCase(StatusParameter(id: event.id));
      result.fold(
          (l) => emit(state.copyWith(
              productsDataState: RequestState.error,
              productsDataMessage: l.message)),
          (r) => emit(state.copyWith(
              productsData: r, productsDataState: RequestState.loaded)));
    });
    on<GetProductDetailsDataEvent>((event, emit) async {
      final result =
          await getProductDetailsDataUseCase(StatusParameter(id: event.id));
      result.fold(
          (l) => emit(state.copyWith(
